...
def test_log_feedback_writes_header_once(tmp_path: Path) -> None:
    ...
    log_feedback(path, question_hash="abc", rating="up", comment="muito bom")
    log_feedback(path, question_hash="def", rating="down", comment="imprec, iso\nlinha nova")
    log_feedback(path, question_hash="ghi", rating="INVALID")  # silenciosamente ignorado
